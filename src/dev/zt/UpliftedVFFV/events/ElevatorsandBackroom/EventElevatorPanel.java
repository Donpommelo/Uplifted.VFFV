...
public class EventElevatorPanel extends Event {
	...
	public void run(){	
		...
		if(!Event.getEvents()[52].isSelfswitch1() && Event.getEvents()[54].isSelfswitch1()){		//if EmployeeIntro has already left + Pen
			...
			case 1:
				if(this.isSelfswitch2()){										//if Pen Pal has been recruited, its event is erased
					Event.getEvents()[52].setDrawn(false);
	...
	public void ChoiceMade(int i){
		...
		else if(!Event.getEvents()[52].isSelfswitch1()){
			if(!Event.getEvents()[54].isSelfswitch1()){
				if(i == 5){
					super.screenShake(50);
					super.setVar(1,5);
					Event.getEvents()[54].run();	
