...
public class EventRedGate extends Event {
	...
	public EventRedGate(float x, float y, int idnum) {
	...
	public void run(){
		...
		case 0:
			if(this.isSelfswitch1()){
				super.transport("/Worlds/Floor6FoodServery/LachiplatreLand/LachiplatreLandBallPit2.txt",5,26,"");
				this.setstage(0);
			}
			else{
				if(balls.size() < 1){					
					Dialog[] d = new Dialog[2];
					d[0] = new Dialog("meep","/CharacterBusts/Arturo.png",1,"A mysterious Red arch with a ball-shaped receptacle.");
					...
					d1[0] = new Dialog("meep","/CharacterBusts/Arturo.png",1,"A mysterious Red arch with a ball-shaped receptacle.");
