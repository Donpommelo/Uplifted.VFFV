...
public class EventManager {
	...
	public void renderBelow(Graphics g){
		for(int y = (int)(game.getGameCamera().getyOffset()/Tile.TILEWIDTH)-5;y<(int)(gs.getPlayer().getPlayerY()/32);y++){
			for(int x = (int)(game.getGameCamera().getxOffset()/Tile.TILEWIDTH)-5;x < (int)(game.getGameCamera().getxOffset()/Tile.TILEWIDTH)+25;x++){
		...
		for(int y = (int)(gs.getPlayer().getPlayerY()/32);y <(int)(game.getGameCamera().getyOffset()/Tile.TILEWIDTH)+18;y++){
			for(int x = (int)(game.getGameCamera().getxOffset()/Tile.TILEWIDTH)-5;x < (int)(game.getGameCamera().getxOffset()/Tile.TILEWIDTH)+25;x++){
				if(x>-1 && x<width && y>-1 && y < height){
					if(events[x][y] != 0 && getEvent(x,y).drawn() && getEvent(x, y).renderBelow() == -1){
						getEvent(x, y).render(g, (int)(x*32 - game.getGameCamera().getxOffset()),(int)(y*32 - game.getGameCamera().getyOffset()));
