...
public class Augur extends Schmuck{
	...
	public final static int[] startStats = {23,30,10,10,11,11,12,17};
...
//		super("Jorge",100,50, 12, 20, 17, 8, 10, 5,ImageLoader.loadImage("/CharacterBusts/3rdSouthOffices-5.png"));
		super("Augur",1,ImageLoader.loadImage("/BattleSprites/Augur.png"), startStats, statGrowths);
		calcStats(1);
...
//		return new Action(this,bs.bp.allies.get((int)(Math.random()*bs.bp.allies.size())),new StandardAttack(0),bs);
		Schmuck target = bs.bp.allies.get((int)(Math.random()*bs.bp.allies.size()));
		int mostStacks = 0;
		...
		if(stacked != -1){
			if(target.statuses.get(stacked).stack*10>=Math.random()*100){
